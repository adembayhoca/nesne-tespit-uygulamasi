...
class VideoTespit(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        # Video ve görüntü işleme değişkenleri
        ...
        # Sayaçlar ve butonlar videonun hemen altında
        self.ust_kontrol_frame = ctk.CTkFrame(self.container)
        self.ust_kontrol_frame.pack(fill=tk.X, padx=10, pady=(0, 5))
        
        # Video yolu girişi
        self.video_path = ctk.CTkEntry(self.ust_kontrol_frame, width=300)
        self.video_path.pack(side=tk.LEFT, padx=5)
        
        # Güven eşiği ayarı - video yolunun yanına ekle
        ...
        self.video_sec_buton = ctk.CTkButton(
            self.ust_kontrol_frame, 
            text="Video Seç",
        ...
        self.video_sec_buton.pack(side=tk.RIGHT, padx=5)
        
        self.baslat_buton = ctk.CTkButton(self.ust_kontrol_frame, text="Başlat",
                                        command=self.tespiti_baslat,
    ...
    def veritabani_olustur(self):
        ...
        try:
            ...
            self.conn = sqlite3.connect("veritabani/video_tespit.db")
    ...
    def video_sec(self):
        """Video dosyası seçme dialogunu açar"""
        ...
        if dosya_yolu:
            ...
            self.video_path.insert(0, dosya_yolu)
    
    def tespiti_baslat(self):
        """Video tespitini başlatır"""
        if not self.video_path.get():
            messagebox.showwarning("Uyarı", "Lütfen önce bir video seçin!")
            return
        ...
        if self.video_capture is None:
            ...
            self.video_sec_buton.configure(state="disabled")
    
    def tespiti_durdur(self):
        """Video tespitini durdurur"""
        self.is_running = False
        ...
        self.durdur_buton.configure(state="disabled")
        self.video_sec_buton.configure(state="normal")
    
    def tespit_dongusu(self):
        """Sürekli olarak videodan görüntü alır ve nesne tespiti yapar"""
    ...
    def tespitleri_kaydet(self):
        ...
        try:
            ...
            self.cursor.execute('''
                ...
                VALUES (?, ?, ?)
            ''', ("Video Tespit Raporu", datetime.now(), self.insan_sayisi))
    ...
    def destroy(self):
        """Frame kapatıldığında videoyu durdur ve veritabanı bağlantısını kapat"""
        self.tespiti_durdur()
...
def icerik_goster(parent_frame):
    return VideoTespit(parent_frame)
